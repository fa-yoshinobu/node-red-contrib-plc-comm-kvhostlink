...
This page summarizes Host Link PLC errors as this Node-RED package reports
them. It is not a complete manufacturer code table; use the KEYENCE manuals for
formal definitions.

`HostLinkError.code` contains the PLC code when the PLC returns an error
response. Connection and malformed-frame failures use separate error classes.
