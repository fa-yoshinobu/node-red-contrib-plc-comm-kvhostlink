...
| Writes | Numeric/string bit values or implicit bit-in-word named writes are rejected. | Supply JavaScript Booleans for direct-bit writes. Call `writeBitInWord` or `writeBitInExpansionUnitBuffer` explicitly only when their two-request, non-PLC-atomic semantics and lost-update risk are acceptable. |
