...
- Library: Named-read and polling plans parse each input address once and reuse immutable internal device/dtype/count/decode metadata. Public parser and result contracts are unchanged.
- Tests: Added targeted compatible-group planning, descending/input-order mapping, aggregate poll admission, UDP reuse/retirement, abort-reason/listener cleanup, response ownership, and public Buffer isolation coverage.
- Docs: Documented optimized read ordering, non-atomic aggregate behavior, reusable UDP endpoints, and response ownership. Public signatures are unchanged, so no API migration step is required.
...
- Library: UDP retains logical connected state while reusing a successful physical socket and local endpoint. Failed active sockets close immediately; replacement-endpoint setup failure remains a definitive pre-send error.
- Tests: Added deterministic TCP pre-write response-race, mixed-format monitor, `Z:F` no-send, canonical hexadecimal, and local UDP endpoint reuse and retirement coverage.
...
- Library: `readNamed` validates and snapshots the complete plan, groups compatible device families in first-appearance order, sorts addresses within each group, merges contiguous ranges, and splits only at protocol limits. It holds one FIFO turn, stops on first failure, preserves input-order result mapping, and exposes no partial result; multi-request reads remain explicitly non-atomic.
