...
- Show human-readable PLC profile labels in the Node-RED editor while continuing to save canonical `keyence:...` profile values.
- Preserve the selected `plcProfile` as connection/client metadata and expose it from `getProfile()`.

### Fixed
- Reject invalid Host Link connection ports before constructing the runtime client.
- Validate Host Link timeout values as finite positive numbers in both the connection node and low-level client.
