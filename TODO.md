...
- [ ] **Unify PLC profile naming across libraries**: Review the public Node-RED connection model selector and align it with the cross-library `PlcProfile` naming policy where practical. Standard saved/displayed names should converge on one canonical lowercase form such as `keyence:kv-x500` or `keyence:kv-7000`, with legacy names accepted only as input aliases. Do not collapse models into a broad profile when device ranges can differ.
