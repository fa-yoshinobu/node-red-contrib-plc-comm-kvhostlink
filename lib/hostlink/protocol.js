...
function buildFrame(body) {
  ...
  if (/[\r\n]/.test(body)) {
    throw new HostLinkProtocolError("Host Link request body must not contain CR or LF");
  }
...
function parseScalarToken(token, options = {}) {
  if (options.packedDirectBitsU16 === true) {
    const text = String(token);
    if (!/^[0-9]{1,5}$/.test(text)) {
      throw new HostLinkProtocolError(
        `Invalid packed direct-bit MWR token '${token}'; expected one to five decimal digits`,
      );
    }
    const parsed = Number(text);
    if (parsed > 0xffff) {
      throw new HostLinkProtocolError(
        `Packed direct-bit MWR token '${token}' is outside the unsigned 16-bit range`,
      );
    }
    return parsed;
  }
  ...
  if ([".U", ".S", ".D", ".L"].includes(options.dataFormat)) {
    const text = String(token);
    const signed = options.dataFormat === ".S" || options.dataFormat === ".L";
    const numericPattern = signed ? /^[+-]?\d+$/ : /^\d+$/;
