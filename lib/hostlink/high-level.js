...
function parseAddress(address) {
  ...
  if (core.includes(".")) {
    ...
    return { base: base.trim(), dtype: "BIT_IN_WORD", bitIndex: Number.parseInt(bitText, 16), count, hasCount, explicitDtype: false };
  }
  const device = parseDevice(core);
  return { base: core, dtype: defaultDtypeForDevice(device.deviceType), bitIndex: null, count, hasCount, explicitDtype: false };
}
...
function formatParsedAddress(parsed) {
  ...
  let text = base;
  const dtype = canonicalizeDtype(parsed.dtype || defaultDtypeForDevice(device.deviceType));
...
async function readTimerCounter(client, device) {
  ...
  const value = await client.read(target, { dataFormat: ".D" });
...
function tryCompileReadNamedPlan(addresses) {
  ...
  for (const bucket of requestsByDeviceType.values()) {
    ...
    for (const request of bucket) {
      ...
      if (!currentStart || requestStart > currentEndExclusive || currentMode !== requestMode) {
        if (currentStart) {
          segments.push({
...
async function writeParsed(client, address, device, parsed, value) {
  if (parsed.dtype === "BIT_IN_WORD") {
    ...
    await writeBitInWord(client, parsed.base, requireBitInWordIndex(address, parsed.bitIndex), Boolean(value));
    return;
  }
  if (parsed.dtype === "COMMENT") {
  ...
  if (parsed.count > 1) {
    ...
    if ((device.deviceType === "T" || device.deviceType === "C") && parsed.dtype !== "BIT") {
      await client.writeSetValueConsecutive(parsed.base, value.map((item) => normalizeWriteScalar(item, parsed.dtype)), {
        dataFormat: `.${parsed.dtype}`,
      });
      return;
    }
