...
async function readComments(client, device, options) {
  return client.readComments(device, options);
}
...
async function readNamed(client, addresses, options) {
  ...
  const execute = () => executeReadNamedPlan(client, plan, options);
  return typeof client._runExclusive === "function" ? client._runExclusive(execute, options) : execute();
}
...
async function* poll(client, addresses, intervalMs, options) {
  ...
  const operationOptions = snapshotOperationOptions(options, "poll");

  while (true) {
    const execute = () => executeReadNamedPlan(client, plan, operationOptions);
    yield typeof client._runExclusive === "function"
      ? await client._runExclusive(execute, operationOptions)
      : await execute();
    await delay(normalizedIntervalMs, operationOptions.signal);
  }
}
...
function compileReadNamedPlan(addresses) {
  ...
  const entries = addresses.map((address, index) => {
    if (seen.has(address)) {
      throw new ValueError(`readNamed contains duplicate address '${address}', which cannot have two result mappings.`);
    }
    seen.add(address);
    const parsed = parseAddress(address);
    const device = parseDevice(parsed.base);
  ...
  const operations = [];
  const optimizableRequests = entries.map((entry) => tryParseOptimizableReadNamedRequest(entry.address, entry.index));
  if (optimizableRequests.some((request) => request === null)) {
    return { entries, operations: entries.map((entry) => ({ kind: "entry", entry })) };
  }
  let segment = null;
  const flushSegment = () => {
    if (!segment) return;
    segment.count = segment.endExclusive - segment.startNumber;
    delete segment.endExclusive;
    operations.push(segment);
    segment = null;
  };

  for (const [entryIndex, entry] of entries.entries()) {
    const request = optimizableRequests[entryIndex];
    ...
    const limit = readPlanSegmentLimit(request.baseAddress.deviceType, mode);
    const canAppend = segment
      && segment.kind === "segment"
      && segment.startAddress.deviceType === request.baseAddress.deviceType
      && segment.mode === mode
      && requestStart >= segment.startNumber
...
async function executeReadNamedPlan(client, plan, options) {
  const resolved = new Array(plan.entries.length).fill(undefined);

  for (const operation of plan.operations) {
    if (operation.kind === "entry") {
      const { entry } = operation;
      resolved[entry.index] = await readParsed(client, entry.address, entry.device, entry.parsed, options);
      continue;
    }
    const segment = operation;
    if (segment.mode === "DIRECT_BITS") {
      const tokens = await client.readConsecutive(deviceToString(segment.startAddress), segment.count, undefined, options);
      ...
      const words = await readWords(client, deviceToString(segment.startAddress), segment.count, options);
      for (const request of segment.requests) {
        const offset = readPlanNumber(request) - segment.startNumber;
...
async function readParsed(client, address, _device, parsed, options) {
  if (parsed.dtype === "BIT_IN_WORD") {
    if (parsed.hasCount) {
    ...
    const word = Number(await readTyped(client, parsed.base, "U", options)) & 0xffff;
  ...
  if (parsed.dtype === "COMMENT") {
    ...
    return readComments(client, parsed.base, options);
  }
  if (parsed.count > 1) {
    if (parsed.dtype === "F") {
  ...
  if (parsed.dtype === "BIT") {
    return readTyped(client, parsed.base, "BIT", options);
  }
  return readTyped(client, parsed.base, parsed.dtype, options);
}
