...
function getBatchableWriteSpec(entry) {
  ...
  if (["U", "S", "H"].includes(parsed.dtype) || (device.deviceType === "AT" && ["D", "L"].includes(parsed.dtype))) {
    return {
      kind: "scalar",
      deviceType: device.deviceType,
      dtype: parsed.dtype,
...
async function writeParsed(client, address, device, parsed, value) {
  ...
  if (parsed.count > 1) {
    ...
    if (device.deviceType === "AT" && (parsed.dtype === "D" || parsed.dtype === "L")) {
