...
async function readNamed(client, addresses, options) {
  ...
  const execute = () => executeReadNamedPlan(client, plan, readOptions);
  return typeof client._runExclusive === "function"
    ? client._runExclusive(execute, readOptions.operationOptions)
    : execute();
}
...
async function executeReadNamedPlan(client, plan, readOptions) {
  ...
  for (const operation of plan.operations) {
    if (operation.kind === "entry") {
      const { entry } = operation;
      resolved[entry.index] = await readParsed(client, entry.address, entry.device, entry.parsed, readOptions);
      continue;
    }
...
async function readParsed(client, address, _device, parsed, readOptions) {
  const options = readOptions.operationOptions;
  if (parsed.dtype === "BIT_IN_WORD") {
    if (parsed.hasCount) {
