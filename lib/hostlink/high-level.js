...
async function writeTyped(client, device, dtype, value) {
  ...
  if (normalized === "F") {
    await client.writeConsecutive(device, encodeFloatWords(Number(value)), ".U");
    return;
  }
  await client.write(device, normalized === "H" ? normalizeHexWriteValue(value) : value, `.${normalized}`);
}
