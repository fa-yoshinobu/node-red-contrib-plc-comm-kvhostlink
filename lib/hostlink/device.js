...
const NATIVE_32BIT_DEVICE_TYPES = new Set(["T", "TC", "TS", "C", "CC", "CS", "Z", "AT"]);
...
const FORCE_SINGLE_DEVICE_TYPES = new Set(["R", "B", "MR", "LR", "CR", "T", "C", "VB", "X", "Y", "M", "L"]);
const FORCE_CONSECUTIVE_DEVICE_TYPES = new Set(["R", "B", "MR", "LR", "CR", "VB", "X", "Y", "M", "L"]);
const FORCE_DEVICE_TYPES = FORCE_SINGLE_DEVICE_TYPES;
...
const WR_DEVICE_TYPES = new Set(Object.keys(DEVICE_RANGES).filter((deviceType) => deviceType !== "AT"));
...
function requireExplicitFormat(device, dataFormat = null) {
  const suffix = dataFormat == null ? device.suffix : normalizeSuffix(dataFormat);
  if (suffix) {
    return suffix;
  }
  const defaultFormat = resolveEffectiveFormat(device.deviceType, "");
  if (!defaultFormat) {
    return "";
  }
  throw new HostLinkProtocolError(
    `Data format suffix is required for device '${deviceToString(device)}'. ` +
    `Specify a suffix such as '${defaultFormat}' in the device string or dataFormat option.`
  );
}
