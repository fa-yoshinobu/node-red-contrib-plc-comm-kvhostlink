...
const PACKED_DIRECT_BITS_U16_RESPONSE = Object.freeze({ packedDirectBitsU16: true });
...
function parseTimerCounterResponseTokens(tokens, dataFormat, device) {
  ...
  return [Number(status), ...parseDataTokens(tokens.slice(1), { dataFormat })];
}
...
class HostLinkClient {
  ...
  async read(device, dataFormat, options) {
    ...
    const decodeValues = deviceType === "T" || deviceType === "C"
      ? (tokens) => parseTimerCounterResponseTokens(tokens, suffix, token)
      : undefined;
   ...
   * admission. After activation, one local FIFO turn and one absolute deadline
   * cover exactly one read and one write, even when the bit already has the
   * requested value. There is no fallback, retry, or success readback. This is
   * not PLC-atomic: PLC logic or another connection can update the word between
   * requests and that update can be lost.
   */
  async writeBitInWord(device, bitIndex, value, options) {
    const operationOptions = normalizeOperationOptions(options, "writeBitInWord");
    validateRange("bitIndex", bitIndex, 0, 15);
    const enabled = normalizeBitWriteValue(value);
    const baseAddress = parseDevice(device);
    if (baseAddress.suffix || resolveEffectiveFormat(baseAddress.deviceType, "") !== ".U") {
      throw new HostLinkProtocolError("writeBitInWord requires an ordinary 16-bit word device");
    }
    const [token, suffix] = this._deviceWithFormat(device, ".U", 1);
    const addr = parseDevice(token);
    validateDeviceType("WR", addr.deviceType, WR_DEVICE_TYPES);
    this._formatValue(0, suffix);
    const readCommand = `RD ${token}`;
    const writePrefix = `WR ${token} `;
    this._buildCommand(readCommand);
    this._buildCommand(`${writePrefix}65535`);

    return this._enqueue(async () => {
      const socket = this._socket;
      const generation = this._generation;
      const response = await this._sendDecodedImmediate(readCommand);
      let current;
      try {
      ...
      const updated = enabled ? current | mask : current & ~mask;
      await this._expectOkImmediate(`${writePrefix}${updated & 0xffff}`);
    }, operationOptions);
  }
   ...
   *
   * The route is fixed to unitNo/address and the format is exactly one
   * unsigned 16-bit word. The complete plan is validated before FIFO
   * admission. One absolute deadline covers exactly one read and one write
   * after activation. This is not PLC-atomic and performs no fallback, retry,
   * or success readback.
   */
  async writeBitInExpansionUnitBuffer(unitNo, address, bitIndex, value, options) {
    const operationOptions = normalizeOperationOptions(options, "writeBitInExpansionUnitBuffer");
    validateRange("unitNo", unitNo, 0, 48);
    validateRange("address", address, 0, 59999);
    validateRange("bitIndex", bitIndex, 0, 15);
    const enabled = normalizeBitWriteValue(value);
    const suffix = requireExpansionDataFormat(".U");
    validateExpansionBufferCount(suffix, 1);
    validateExpansionBufferSpan(address, suffix, 1);
    const route = `${unitNo.toString().padStart(2, "0")} ${address}${suffix}`;
    const readCommand = `URD ${route} 1`;
    const writePrefix = `UWR ${route} 1 `;
    this._buildCommand(readCommand);
    this._buildCommand(`${writePrefix}65535`);
    ...
    return this._enqueue(async () => {
      ...
      try {
        ...
        if (tokens.length !== 1) {
          throw new HostLinkProtocolError(
            `URD response token count mismatch: expected 1, received ${tokens.length}`,
          );
        }
        const values = parseDataTokens(tokens, { dataFormat: suffix });
        current = values.length === 1
          && Number.isSafeInteger(values[0])
          && values[0] >= 0
          && values[0] <= 0xffff
  ...
  async registerMonitorWords(entries, options) {
    ...
    const tokens = targets.map((entry) => {
      ...
      // Host Link keeps the registration token bare, but MWR returns the
      // 16-bit packed word beginning at a bare direct-bit target.
      formats.push(format === "" && DIRECT_BIT_DEVICE_TYPES.has(addr.deviceType)
        ? PACKED_DIRECT_BITS_U16_RESPONSE
        : format);
      return token;
    });
    await this._enqueue(async () => {
    ...
    return new Promise((resolve, reject) => {
      const entry = {
        ...
        detachAbort() {
          ...
          if (this.signal && handler) this.signal.removeEventListener("abort", handler);
        },
      };
      if (signal) {
        entry.abortHandler = () => {
          ...
          try {
            ...
            // Signal cleanup must not prevent cancellation settlement or FIFO progress.
          }
          try {
        ...
        try {
          ...
          try {
            ...
            // Preserve the registration failure as the operation result.
          }
          reject(error);
          return;
        }
  ...
  _activateNextOperation() {
    ...
    while (!this._activeOperation && this._operationQueue.length > 0) {
      ...
      try {
        ...
        if (entry.signal) {
          ...
          if (entry.signal.aborted) {
            ...
            let activeSignal = entry.signal;
            let activeHandler = forwardCallerAbort;
            entry.detachActiveSignals = () => {
              const signalToDetach = activeSignal;
              const handlerToDetach = activeHandler;
              activeSignal = null;
              activeHandler = null;
              if (signalToDetach && handlerToDetach) {
                signalToDetach.removeEventListener("abort", handlerToDetach);
              }
            };
        ...
        const context = {
          ...
          deadline: performance.now() + this.timeout,
          transaction: null,
        };
        ...
        Promise.resolve()
          ...
          .finally(() => {
            try {
              ...
              // Cleanup failure must not change the settled operation or surface unhandled rejection.
            }
        ...
        try {
          ...
          // Setup failure remains primary; later FIFO work must still progress.
        }
        try {
          entry.detachAbort();
        } catch (_detachError) {
          // A broken signal cannot be allowed to strand later FIFO work.
        }
        if (this._activeOperation === entry) this._activeOperation = null;
        entry.reject(error);
      }
  ...
  async _sendExpectedTokens(body, expectedCount, dataFormat, command, options, decodeValues) {
    return this._enqueue(async () => {
      const socket = this._socket;
      const generation = this._generation;
      ...
      try {
        ...
        const result = decodeValues
          ? decodeValues(tokens)
          : Array.isArray(dataFormat)
            ? tokens.map((token, index) => {
              const tokenFormat = dataFormat[index];
              const parseOptions = tokenFormat && typeof tokenFormat === "object"
                ? tokenFormat
                : { dataFormat: tokenFormat };
              return parseDataTokens([token], parseOptions)[0];
            })
