...
class HostLinkClient {
  ...
  async confirmOperatingMode() {
    const response = await this._sendDecoded("?M");
  ...
  async read(device, dataFormat) {
    ...
    const response = await this._sendDecoded(`RD ${token}`);
  ...
  async readConsecutive(device, count, dataFormat) {
    ...
    const response = await this._sendDecoded(`RDS ${token} ${count}`);
  ...
  async readExpansionUnitBuffer(unitNo, address, count, dataFormat) {
    ...
    const response = await this._sendDecoded(`URD ${Number(unitNo).toString().padStart(2, "0")} ${Number(address)}${suffix} ${count}`);
    return parseDataTokens(splitDataTokens(response), { dataFormat: suffix });
  }
  ...
  async readMonitorBits() {
    return parseDataTokens(splitDataTokens(await this._sendDecoded("MBR")));
  }

  async readMonitorWords() {
    return splitDataTokens(await this._sendDecoded("MWR"));
  }
  ...
  async writeBitInWord(device, bitIndex, value) {
    ...
    return this._enqueue(async () => {
      ...
      const values = parseDataTokens(splitDataTokens(response), { dataFormat: suffix });
      if (values.length !== 1 || !Number.isInteger(values[0]) || values[0] < 0 || values[0] > 0xffff) {
  ...
  async _expectOk(body) {
    const response = await this._sendDecoded(body);
    ...
    while (true) {
      ...
      if (indexes.length === 0) {
        if (this._receiveBuffer.length > ABSOLUTE_RESPONSE_CAP) {
          this._failTcpConnection(new HostLinkProtocolError(`Response line exceeds ${ABSOLUTE_RESPONSE_CAP} bytes`));
        }
        return;
      }
