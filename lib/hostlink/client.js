...
class HostLinkClient {
  constructor(options = {}) {
    ...
    this.bufferSize = Number(options.bufferSize === undefined ? DEFAULT_BUFFER_SIZE : options.bufferSize);
    ...
    if (!Number.isInteger(this.bufferSize) || this.bufferSize <= 0) {
      throw new ValueError(`bufferSize must be a positive integer: ${options.bufferSize}`);
    }
  ...
  _handleTcpData(chunk) {
    ...
    while (true) {
      ...
      if (indexes.length === 0) {
        if (this._receiveBuffer.length > this.bufferSize) {
          this._failTcpConnection(new HostLinkProtocolError(`Response line exceeds ${this.bufferSize} bytes`));
        }
        return;
      }
    ...
    return new Promise((resolve, reject) => {
      ...
      this._socket.write(payload, (error) => {
        if (error) {
          ...
          this._failTcpConnection(wrapped);
          reject(wrapped);
          return;
        }
        resolve();
  ...
  _writeUdpAndRead(payload) {
    return new Promise((resolve, reject) => {
      ...
      const onError = (error) => finish(new HostLinkConnectionError(`Socket communication failed: ${error.message}`));
      const timeoutHandle = setTimeout(() => {
        if (this._socket === socket) {
          this._socket = null;
        }
        finish(new HostLinkConnectionError("Timeout while waiting response from PLC"));
        try {
          socket.close();
        } catch (_error) {
      ...
      socket.send(payload, (error) => {
        if (error) {
          finish(new HostLinkConnectionError(`Socket communication failed: ${error.message}`));
        }
      });
    });
  }
