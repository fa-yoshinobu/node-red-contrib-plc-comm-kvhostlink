...
const { buildFrame, decodeResponse, ensureSuccess, parseDataTokens, splitDataTokens } = require("./protocol");
...
class HostLinkClient {
  ...
  async readComments(device, options = {}) {
    ...
    const response = await this.sendRaw(`RDC ${this._deviceToken(device, { dropSuffix: true })}`);
  ...
  _processResponse(response) {
    return ensureSuccess(decodeResponse(response));
  }
