...
class HostLinkClient {
  ...
  async _prepareUdpRequestSocket(deadline, signal) {
    ...
    if (this._socket && !this._udpSocketUsed) {
    ...
    const address = this._udpAddress || this.host;
    const socket = dgram.createSocket("udp4");
    await new Promise((resolve, reject) => {
      ...
      socket.connect(this.port, address, () => {
        ...
        const currentEndpoint = socket.address();
        if (previousSocket && typeof previousSocket.address === "function") {
          try {
            const previousEndpoint = previousSocket.address();
            if (
              previousEndpoint.address === currentEndpoint.address
              && previousEndpoint.port === currentEndpoint.port
            ) {
              finalizeReject(new HostLinkConnectionError("UDP request socket reused the retained local endpoint"));
              return;
            }
          } catch (_addressError) {
            // A stale retained socket may already have closed; the new endpoint is still isolated.
          }
        }
        connecting.settled = true;
        clearTimeout(connecting.timeoutHandle);
        ...
        if (previousSocket) {
          try {
            ...
            // A stale retained socket may already have closed.
          }
        }
        resolve();
      });
